...
## Ki — the enforcement problem in multi-party travel
...
## Shō — what policy enforcement usually looks like
...
**Mandate verification.** Every action taken by an AI agent carries an ATP Mandate JWT — a cryptographically signed token that declares the agent's identity and permitted authority scope. The Security Kernel verifies the signature (Ed25519/EdDSA) and evaluates the Cedar policy set embedded in the mandate against the proposed action. An agent whose mandate does not include the action it is attempting is rejected. An agent whose mandate is bound to Booking Object A cannot act on Booking Object B. These are hard rejections, not warnings.
...
## The enforcement stack in sequence

For any action taken by an AI agent via the MCP Server, the enforcement sequence is:

1. **Windley Loop** (before planning): Cedar partial evaluation against the agent's mandate produces a permitted action space. The agent plans within this space. Actions outside it are not attempted.
...
## Ketsu — what this gives the industry

The Security Kernel is not primarily a security feature in the traditional IT sense. It is a governance mechanism.
...
The Security Kernel is the component that makes it technically accurate to say: no unverified, unauthorised action can change the state of a protocol-compliant booking. That is a strong claim. It is the one the protocol is designed to keep.
